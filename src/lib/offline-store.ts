...
export async function getCachedResponse(
  ...
  try {
    return (
      (await store<CachedResponse | undefined>(CACHE_STORE, "readonly", (s) =>
        s.get(responseCacheKey(userId, url)),
      )) ?? null
    );
  } catch {
    return null;
...
export async function clearOfflineDataForUser(userId: string): Promise<void> {
  ...
  try {
    const db = await openDb();
    await Promise.all([
      clearMatching(db, CACHE_STORE, userId),
      clearMatching(db, QUEUE_STORE, userId),
    ]);
    emitQueueChange();
  } catch {
    void 0;

...
export function ModerationPage() {
  ...
  const act = async (id: string, action: "approve" | "reject" | "duplicate") => {
    try {
      ...
      queryClient.setQueryData<{ sales: any[] }>(["mod-list"], (current) =>
        current ? { ...current, sales: current.sales.filter((sale) => sale.id !== id) } : current,
      );
          ...
          <Card key={r.id}>
            <CardContent className="p-4 flex flex-col md:flex-row md:items-center gap-3">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <Button size="sm" variant="outline" onClick={() => act(r.id, "duplicate")}>
                  Дублікат
                </Button>

...
async function fetchSales(req: Request, env: Env) {
  ...
  if (f.error) return err(400, f.error, env, req);
...
export async function summary(req: Request, env: Env): Promise<Response> {
  ...
  const topDistricts = Object.entries(districtCounts)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([k, v]) => ({ district: k, count: v }));
  const withFinalPrice = prices.length;
  const discounts = rows
    .filter(
      (r: any) =>
        r.initial_price != null &&
        Number(r.initial_price) > 0 &&
        Number.isFinite(Number(r.final_price)),
    )
    .map(
      (r: any) =>
        ((Number(r.initial_price) - Number(r.final_price)) / Number(r.initial_price)) * 100,
    );
  return json(
    {
      total: rows.length,
...
export async function byDistrict(req: Request, env: Env): Promise<Response> {
  ...
  const data = Object.keys(m).map((d) => ({
    district: d,
    count: m[d].length,
    avg_price: avg(m[d]),
  }));
  ...
  return json(
    {
      avg: avg(ds),
      median: median(ds),
      distribution: labels.map((l, i) => ({ bucket: l, count: counts[i] })),
    },
    {},
    env,
    req,
...
export async function distribution(req: Request, env: Env): Promise<Response> {
  ...
  for (let i = 0; i < buckets; i++) {
    ...
    data.push({
      ...
      count: prices.filter((p) => p >= lo && (i === buckets - 1 ? p <= hi : p < hi)).length,
    });
  }
  return json({ data }, {}, env, req);
}

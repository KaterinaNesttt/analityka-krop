...
export function corsHeaders(env: Env, req: Request): Record<string, string> {
  ...
  const allowedOrigins = [
    ...new Set([...configuredOrigins.filter((o) => o !== "*"), "http://localhost:8080"]),
  ];
  ...
  if (origin && (allowAny || allowedOrigins.includes(origin))) {
    headers["Access-Control-Allow-Origin"] = allowAny ? "*" : origin;
  }
  return headers;
}
...
async function cacheVersion(env: Env, entity: string): Promise<number> {
  try {
    const row = await env.DB.prepare("SELECT version FROM cache_versions WHERE entity = ?")
      .bind(entity)
...
async function cacheHeaders(
  ...
  const headers = new Headers({
    ETag: etagFor(
      entity,
      await cacheVersion(env, entity),
      `${url.pathname}?${canonicalQuery}|${variant}`,
    ),
...
export async function notModified(
  entity: string,
  env: Env,
  req: Request,
  variant = "",

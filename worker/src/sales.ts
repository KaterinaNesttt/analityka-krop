...
export function parseFilters(url: URL): SaleFilters {
  ...
  const num = (k: string) => {
    ...
    if (!Number.isFinite(n)) {
      f.error = `Невірне значення "${k}"`;
      return undefined;
    }
    return n;
  };
  f.district = get("district");
  f.districts = [...p.getAll("districts"), ...p.getAll("districts[]")]
    .map((v) => v.trim())
    .filter(Boolean);
  f.price_min = num("price_min");
  f.price_max = num("price_max");
  f.status = get("status");
  f.floor = get("floor");
...
function parseLimit(url: URL): { limit: number; error?: string } {
  const raw = url.searchParams.get("limit")?.trim();
  if (!raw) return { limit: 200 };
  const limit = Number(raw);
  if (!Number.isInteger(limit) || limit < 1)
    return { limit: 200, error: 'Невірне значення "limit"' };
...
export function buildWhere(f: SaleFilters, forUser: boolean): { sql: string; params: any[] } {
  ...
  if (f.districts?.length) {
    w.push(`district IN (${f.districts.map(() => "?").join(", ")})`);
    params.push(...f.districts);
  } else if (f.district) {
    w.push("district = ?");
...
export async function listSales(req: Request, env: Env): Promise<Response> {
  ...
  const cached = await notModified("sales", env, req, u.role);
  if (cached) return cached;
  const forUser = u.role === "user";
  const url = new URL(req.url);
  const filters = parseFilters(url);
  if (filters.error) return err(400, filters.error, env, req);
  const limitResult = parseLimit(url);
  if (limitResult.error) return err(400, limitResult.error, env, req);
  ...
  const { results } = await env.DB.prepare(
    `SELECT ${fields} FROM sales${sql} ORDER BY ${orderBy} LIMIT ?`,
  )
    .bind(...params, limitResult.limit)
    .all();
...
export async function getSale(req: Request, env: Env, id: string): Promise<Response> {
  ...
  const forUser = u.role === "user";
  const fields = forUser ? PUBLIC_FIELDS : STAFF_FIELDS;
  const row = await env.DB.prepare(`SELECT ${fields} FROM sales WHERE id = ?`)
    .bind(id)
    .first<any>();
...
export async function createSale(req: Request, env: Env): Promise<Response> {
  ...
  for (const k of required)
    if (body[k] === undefined || body[k] === null || body[k] === "")
      return err(400, `Поле "${k}" обовʼязкове`, env, req);
  const id = uid();
  const now = nowIso();
  ...
  await env.DB.prepare(
    ...
    .bind(
      ...
      clean(body.district),
      clean(body.floor),
      clean(body.characteristics),
      clean(body.sale_term),
      initialPrice,
      finalPrice,
      clean(body.comment),
      status,
...
export async function updateSale(req: Request, env: Env, id: string): Promise<Response> {
  ...
  params.push(nowIso());
  params.push(id);
  const result = await env.DB.prepare(`UPDATE sales SET ${sets.join(", ")} WHERE id = ?`)
    .bind(...params)
    .run();
...
export async function changeStatus(
  ...
  if (u instanceof Response) return u;
  const now = nowIso();
  const result = await env.DB.prepare("UPDATE sales SET status = ?, updated_at = ? WHERE id = ?")
    .bind(status, now, id)
    .run();
  if (!Number((result.meta as { changes?: number }).changes ?? 0))
    return err(404, "Не знайдено", env, req);

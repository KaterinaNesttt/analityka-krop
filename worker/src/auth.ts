...
type AuthRateAction = "register" | "login" | "refresh";
const AUTH_RATE_LIMITS: Record<AuthRateAction, { max: number; windowMs: number }> = {
...
function authRateLimit(req: Request, env: Env, action: AuthRateAction): Response | null {
  const now = Date.now();
  ...
  if (!bucket || bucket.resetAt <= now) {
    authRateBuckets.set(key, { count: 1, resetAt: now + limit.windowMs });
    return null;
  }
  bucket.count += 1;
  if (bucket.count > limit.max) return err(429, "Забагато спроб. Спробуйте пізніше.", env, req);
  return null;
}

async function issueTokens(env: Env, userId: string, role: string, status: string) {
  const access = await signJwt(
    { sub: userId, role: role as any, status: status as any },
    env.JWT_SECRET,
    ACCESS_TTL,
  );
...
export async function register(req: Request, env: Env): Promise<Response> {
  const limited = authRateLimit(req, env, "register");
  ...
  return json(
    {
      ...
      name,
      message: isFirst
        ? "Створено першого адміністратора"
        : "Заявку відправлено. Очікуйте підтвердження адміністратором.",
    },
    { status: 201 },
    env,
    req,
...
export async function login(req: Request, env: Env): Promise<Response> {
  const limited = authRateLimit(req, env, "login");
  if (limited) return limited;
  const body = (await req.json().catch(() => null)) as any;
...
export async function refresh(req: Request, env: Env): Promise<Response> {
  const limited = authRateLimit(req, env, "refresh");
  if (limited) return limited;
  const body = (await req.json().catch(() => null)) as any;
